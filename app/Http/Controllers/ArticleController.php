...
class ArticleController extends Controller
{
  ...
  public function index(Request $request)
  {
      ...
      $articles = Article::on($connection)
          ->with(['schoolClass:grade_level,grade_name', 'subject:id,subject_name', 'semester:id,semester_name', 'keywords:id,keyword'])
          ->select(['id', 'title', 'status', 'visit_count', 'created_at', 'grade_level', 'subject_id', 'semester_id'])
          ->latest('created_at')
          ->paginate(25);
  ...
  public function show(Request $request, $id)
  {
      ...
      $article = Article::on($connection)
          ...
          ->select([
              ...
              'visit_count', 'created_at', 'updated_at', 'grade_level',
              'subject_id', 'semester_id', 'author_id', 'status'
          ])
          ->findOrFail($id);
