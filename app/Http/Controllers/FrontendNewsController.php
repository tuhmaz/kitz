  ...
  public function index(Request $request)
{
    ...
    // Get recent news for sidebar/recent section
    $recentNews = News::on($database)
        ->with('category')
        ->orderBy('created_at', 'desc')
        ->limit(5)
        ->get();

    return view('content.frontend.news.index', compact('news', 'categories', 'database', 'recentNews'));
}
