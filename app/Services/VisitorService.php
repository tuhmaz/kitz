...
class VisitorService
{
    ...
    /**
     * جلب بيانات الموقع الجغرافي من عنوان IP عبر geoplugin.net المجاني فقط.
     *
     * @param  string  $ip
     * @return array
     */
    public function getGeoDataFromIP($ip)
    {
        try {
            ...
            if ($error) {
                Log::warning("cURL error for geoplugin.net: " . $error);
            }
            
            $geoData = json_decode($response, true);
            
            if ($geoData && isset($geoData['geoplugin_status']) && $geoData['geoplugin_status'] == 200) {
                $data = [
                    'country' => $geoData['geoplugin_countryName'] ?? 'Unknown',
                    'city'    => $geoData['geoplugin_city'] ?? 'Unknown',
                    'lat'     => $geoData['geoplugin_latitude'] ?? null,
                    'lon'     => $geoData['geoplugin_longitude'] ?? null,
                ];
            ...
            // إذا فشل الاستدعاء
            Log::warning("GeoPlugin failed for IP $ip");
            return [
                'country' => 'Unknown',
                'city'    => 'Unknown',
                'lat'     => null,
